...
const OED_HAS_PLOTS = Ref(false)

function _try_load_plots()
    try
        @eval using Plots
        OED_HAS_PLOTS[] = true
    catch
        @warn "Plots.jl not available. CSV files will still be written, but figures will not be generated."
        OED_HAS_PLOTS[] = false
    end
end

function deterministic_outlet(; Y_in, Temp, ratio, St, k, nref=2500)
    nspec = size(St, 2)
    d = main(
        nref=nref,
        inlet_MFs=Y_in,
    ...
    return youts(d, Nspec=nspec)
end

function sensitivity_matrix(; Y_design, T_design, ratio, St, k, nref=2500, relstep=1e-4)
    nspec = size(St, 2)
    nparas = length(k)
...
function fisher_information_matrix(; Y_design, T_design, ratio, St, k, σ_data, nref=2500)
    nspec = size(St, 2)
    Nexps = size(Y_design, 2)
    ...
    Γinv = (1 / σ_data^2) * I(nspec * Nexps)

    return J' * Γinv * J
end

function global_information_matrix(; Y_candidates, T_candidates, ratio, St, k, σ_data, nref=2500)
    nparas = length(k)
    G = zeros(nparas, nparas)

    for i in 1:size(Y_candidates, 2)
        Yi = reshape(Y_candidates[:, i], :, 1)
        Ti = [T_candidates[i]]
...
function greedy_optimal_design(; Y_candidates, T_candidates, ratio, St, k, σ_data,
                               Nexps_select, nref=2500, criterion=:D)
    Ncand = size(Y_candidates, 2)
    selected = Int[]
    ...
    for step in 1:Nexps_select
        best_idx = remaining[1]
        best_score = -Inf
        best_F = zeros(nparas, nparas)

        for idx in remaining
            Yi = reshape(Y_candidates[:, idx], :, 1)
            Ti = [T_candidates[idx]]

            Fi = fisher_information_matrix(
                Y_design=Yi,
                T_design=Ti,
                ratio=ratio,
                St=St,
            ...
            score =
                criterion == :D ? logdet(Symmetric(Ftest + 1e-12I)) :
                criterion == :A ? -tr(inv(Ftest + 1e-12I)) :
                criterion == :E ? minimum(eigvals(Symmetric(Ftest))) :
                error("Unknown criterion. Use :D, :A, or :E.")
        ...
        F = best_F
        push!(history, best_score)
    end

    return selected, Matrix(F), history
end

function parameter_covariance_from_fim(FIM)
    return inv(FIM + 1e-12I)
end

function write_oed_results(; selected, fim_history, error_history, uncertainty_history)
    writedlm("oed_selected_experiments.csv", selected, ',')
    writedlm("oed_fim_score_history.csv", fim_history, ',')
    writedlm("oed_parameter_error_history.csv", error_history, ',')
    writedlm("oed_uncertainty_history.csv", uncertainty_history, ',')
end

# function plot_oed_results(; fim_history, error_history, uncertainty_history)
#     _try_load_plots()
...
#     p1 = plot(
#         1:length(fim_history),
#         fim_history,
#         xlabel="Number of selected experiments",
#         ylabel="log(det(FIM))",
#         title="Fisher Information Improvement",
#         marker=:circle,
#         legend=false
#     )
#     savefig(p1, "oed_fim_improvement.png")

#     p2 = plot(
#         1:length(error_history),
#         error_history,
#         xlabel="Number of selected experiments",
#         ylabel="Relative parameter error",
#         title="Parameter Estimation Improvement",
#         marker=:circle,
#         legend=false
#     )
#     savefig(p2, "oed_parameter_error.png")

#     p3 = plot(
#         1:length(uncertainty_history),
#         uncertainty_history,
#         xlabel="Number of selected experiments",
...
#         legend=false
#     )
#     savefig(p3, "oed_uncertainty_reduction.png")
...
function plot_oed_results(; fim_history, error_history, uncertainty_history)
    ...
    try
        ...
        @warn "Plots.jl could not be loaded"
        @warn err
        return nothing
    end

    x1 = collect(1:length(fim_history))
    x2 = collect(1:length(error_history))
    x3 = collect(1:length(uncertainty_history))
    ...
    Plots.savefig(p1, "oed_fim_improvement.png")
    ...
    Plots.savefig(p2, "oed_parameter_error.png")
    ...
    Plots.savefig(p3, "oed_uncertainty_reduction.png")

    return p1, p2, p3
end

function complete_workflow_optimal(; Nexps=5,
                                   Ncandidates=15,
                                   ratio=0.1,
                                   nparas=2,
                                   std_data=1e-6,
                                   RBS_full=false,
                                   use_optimal_design=true,
                                   nref=2500,
                                   k_true=[4000.0, 4000.0],
                                   IG=fill(0.1, 2),
                                   N_repeats=50)
    ...
    if !use_optimal_design
        Y_in, Temp = random_points_generator(
            Nexps=Nexps,
            nspecs=nspec,
            lb=lb,
            ub=ub,
    ...
    @info "Generating candidate experiments"
    Y_candidates, T_candidates = random_points_generator(
        Nexps=Ncandidates,
        nspecs=nspec,
        lb=lb,
        ub=ub,
    ...
    @info "Computing global information matrix"
    GIM = global_information_matrix(
    ...
    @info "Selecting optimal experiments"
    selected, FIM, fim_history = greedy_optimal_design(
        Y_candidates=Y_candidates,
        T_candidates=T_candidates,
        ratio=ratio,
        St=St,
    ...
    for m in 1:Nexps
        ...
        T_design = T_candidates[idx]

        @info "Generating synthetic experimental data for $m selected experiments"
        Y_out = experiments(
            Y_in=Y_design,
            Temp=T_design,
            P_total=P_total,
        ...
        @info "Estimating parameters using $m selected experiments"
        k_est = parameter_estimator(
            ratio=ratio,
            nspec=nspec,
            Y_in=Y_design,
        ...
        push!(param_history, collect(k_est))

        rel_error = norm(k_est .- k_true) / norm(k_true)
        push!(error_history, rel_error)

        Fm = fisher_information_matrix(
            Y_design=Y_design,
            T_design=T_design,
            ratio=ratio,
            St=St,
        ...
        C = parameter_covariance_from_fim(Fm)
        push!(uncertainty_history, tr(C))
    end
    ...
    return (
        k_estimate=param_history[end],
        parameter_history=param_history,
        selected_experiments=selected,
        FIM=FIM,
        GIM=GIM,
        fim_history=fim_history,
        parameter_error_history=error_history,
        uncertainty_history=uncertainty_history,
        plots=plots
    )
end
