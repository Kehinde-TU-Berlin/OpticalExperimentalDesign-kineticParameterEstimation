...
function linear_interpolate_oed(x, y, xq)
    if xq <= x[1]
        return y[1]
    elseif xq >= x[end]
        return y[end]
...
function plot_oed_comparison(results; k_true, Nexps)
    ...
    p_err = plot_grouped_by_noise(
        results,
        row -> row.error_history,
        "Relative parameter error",
        "Parameter Estimation Error",
        "cfd_oed_parameter_error.png";
        Nexps=Nexps,
        add_5percent=true
    )

    p_fim = plot_grouped_by_noise(
        results,
        row -> row.fim_history,
        "log(det(FIM))",
        "Fisher Information Growth",
        "cfd_oed_fim_growth.png";
        Nexps=Nexps
    )

    p_unc = plot_grouped_by_noise(
        results,
        row -> row.uncertainty_history,
        "trace(inv(FIM))",
        "Parameter Uncertainty Reduction",
        "cfd_oed_uncertainty_reduction.png";
        Nexps=Nexps
    )
    ...
    p_k1 = plot_grouped_by_noise(
        ...
        "Estimated k₁",
        "Pre-exponential Factor k₁ Convergence",
        "cfd_oed_k1_convergence.png";
        Nexps=Nexps
    )
    ...
    p_k2 = plot_grouped_by_noise(
        ...
        "Estimated k₂",
        "Activation Energy k₂ Convergence",
        "cfd_oed_k2_convergence.png";
        Nexps=Nexps
    )
...
function plot_thesis_style_convergence(results; k_true, Nexps)
    ...
    method_style = Dict(
        :Random => (color=:gray, marker=:diamond, linestyle=:dot),
        :FIM    => (color=:blue, marker=:circle, linestyle=:solid),
        :GIM    => (color=:red, marker=:square, linestyle=:dash)
    )
    ...
    for par_id in 1:2
        ...
        for σ in noises
            ...
            for row in rows
                ...
                # smooth visual curve using dense linear interpolation
                x_dense = range(1, Nexps, length=200)
                y_dense = [linear_interpolate_oed(x, y, xd) for xd in x_dense]
        ...
        title_text = par_id == 1 ?
            "Figure: Convergence of Pre-exponential Factor k₁" :
            "Figure: Convergence of Activation Energy k₂"

        filename = par_id == 1 ?
            "thesis_k1_smooth_convergence_all_noise.png" :
            "thesis_k2_smooth_convergence_all_noise.png"
...
function cfd_oed_parameter_estimator_workflow(; Nexps=7,
    ...
    plot_thesis_style_convergence(all_results; k_true=k_true, Nexps=Nexps)
