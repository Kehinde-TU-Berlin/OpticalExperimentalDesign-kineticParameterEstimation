...
function plot_oed_comparison(results; k_true, Nexps)
    ...
    function make_plot(xlabel, ylabel, title)
        return Plots.plot(
            xlabel = xlabel,
            ylabel = ylabel,
            title = title,
    ...
    p_err = make_plot("Number of experiments", "Relative parameter error", "Parameter Estimation Error")
    p_fim = make_plot("Number of experiments", "log(det(FIM))", "Fisher Information Growth")
    p_unc = make_plot("Number of experiments", "trace(inv(FIM))", "Parameter Uncertainty Reduction")
    p_k1  = make_plot("Number of experiments", "Estimated k₁", "Convergence of k₁")
    p_k2  = make_plot("Number of experiments", "Estimated k₂", "Convergence of k₂")

    for row in results
        label = "$(row.method), σ=$(row.noise)"

        Plots.plot!(p_err, x, row.error_history, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_fim, x, row.fim_history, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_unc, x, row.uncertainty_history, marker=:circle, linewidth=2.5, markersize=5, label=label)

        k1 = [v[1] for v in row.k_history]
        k2 = [v[2] for v in row.k_history]

        Plots.plot!(p_k1, x, k1, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_k2, x, k2, marker=:circle, linewidth=2.5, markersize=5, label=label)
    end

    Plots.hline!(p_k1, [k_true[1]], linestyle=:dash, linewidth=3, label="true k₁")
    Plots.hline!(p_k2, [k_true[2]], linestyle=:dash, linewidth=3, label="true k₂")
...
function plot_extra_oed_report_graphs(results; k_true, Nexps, Jlist, noise_target=1e-4)
    ...
    p_A_error = make_plot(
        ...
        "Relative error in pre-exponential factor k₁",
        "Pre-exponential Factor Error Reduction"
    )

    p_Ea_error = make_plot(
        "Number of experiments",
        "Relative error in activation energy k₂",
        "Activation Energy Error Reduction"
    )

    p_gain = make_plot(
        "Experiment number",
        "Marginal information gain Δlog(det(FIM))",
        "Marginal Information Gain per Experiment"
    )

    p_A_conv = make_plot(
        "Number of experiments",
        "Estimated pre-exponential factor k₁",
        "Pre-exponential Factor Convergence"
    )

    p_Ea_conv = make_plot(
        "Number of experiments",
        "Estimated activation energy k₂",
        "Activation Energy Convergence"
    )

    p_sequence = make_plot(
        "Selection order",
    ...
    p_ellipse = Plots.plot(
        ...
        title="Parameter Confidence Ellipses",
        size=(1500, 900),
        dpi=300,
        legend=:outerright,
        legendfontsize=8,
    ...
    needed_methods = String[]
    needed_values = Float64[]
    ...
    for row in results
        ...
        k1_error = abs.((k1 .- k_true[1]) ./ k_true[1])
        k2_error = abs.((k2 .- k_true[2]) ./ k_true[2])

        marginal_gain = [row.fim_history[1]; diff(row.fim_history)]

        Plots.plot!(p_A_error, x, k1_error, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_Ea_error, x, k2_error, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_gain, x, marginal_gain, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_A_conv, x, k1, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_Ea_conv, x, k2, marker=:circle, linewidth=2.5, markersize=5, label=label)
        Plots.plot!(p_sequence,x,Float64.(row.selected),marker=:circle,linewidth=2.5,markersize=5,label=label)

        if row.noise == noise_target
            total_error = row.error_history
            idxs = findall(e -> e <= 0.05, total_error)

            push!(needed_methods, string(row.method))
            push!(needed_values, isempty(idxs) ? NaN : Float64(first(idxs)))
        end
        ...
        Plots.plot!(p_ellipse, ex, ey, linewidth=2.5, label=label)
        Plots.scatter!(p_ellipse, [final_k[1]], [final_k[2]], label=false, markersize=4)
    end

    Plots.hline!(p_A_error, [0.05], linestyle=:dash, linewidth=2.5, label="5% error")
    Plots.hline!(p_Ea_error, [0.05], linestyle=:dash, linewidth=2.5, label="5% error")

    Plots.hline!(p_A_conv, [k_true[1]], linestyle=:dash, linewidth=3, label="true k₁")
    Plots.hline!(p_Ea_conv, [k_true[2]], linestyle=:dash, linewidth=3, label="true k₂")

    Plots.scatter!(p_ellipse, [k_true[1]], [k_true[2]], marker=:star5, markersize=9, label="true parameter")

    valid_needed = [isnan(v) ? Nexps + 1 : v for v in needed_values]
    ...
    Plots.savefig(p_A_error, "cfd_oed_pre_exponential_error_reduction.png")
    Plots.savefig(p_Ea_error, "cfd_oed_activation_energy_error_reduction.png")
    Plots.savefig(p_gain, "cfd_oed_marginal_information_gain.png")
    Plots.savefig(p_A_conv, "cfd_oed_pre_exponential_convergence.png")
    Plots.savefig(p_Ea_conv, "cfd_oed_activation_energy_convergence.png")
    Plots.savefig(p_sequence, "cfd_oed_selection_sequence.png")
