...
# Include the optimal design module
...
function complete_workflow_optimal(; Nexps=5, ratio=0.1, nparas=2, std_data=1e-6, 
                                   RBS_full=false, use_optimal_design=true)
    
    if use_optimal_design
        # Run optimal design workflow
        ...
        savefig(plot, "optimal_design_complete.png")
        
        return k, unc_history[end], history
    else
        # Original random design
        return complete_workflow(Nexps=Nexps, ratio=ratio, nparas=nparas, 
                                std_data=std_data, RBS_full=RBS_full)
    end
end
